-- Recreate slabs views with sale fields; add ebay_sales_enriched
...
-- ebay_sales_enriched: join master_cards for card_name fallback when parsed card_name is null
create or replace view ebay_sales_enriched as
select
  es.*,
  coalesce(es.card_name, mc.card_name) as display_card_name
from ebay_sales es
left join master_cards mc on mc.set_abbr = es.set_abbr and mc.num = es.num and mc.lang = coalesce(es.lang, 'ENG');
